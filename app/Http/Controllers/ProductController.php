...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        //
        $product = new Product();
        $product->p_name_fr = $request->p_name_fr;
        $product->p_name_ar = $request->p_name_ar;
        ...
        $img = $request->file('p_3rd_img')->getClientOriginalName();
        $path = $request->file('p_3rd_img')->storeAs('product/icons',$img,'img');
        $product->p_3rd_img = $path;

        $product->save();
        return redirect()->route('product_insert');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $img = $request->file('p_3rd_img')->getClientOriginalName();
        $path_p_3rd_img = $request->file('p_3rd_img')->storeAs('product/icons',$img,'img');

        DB::table('products')->where('id',$id)->update([
            'p_name_fr' => $request->p_name_fr,
            'p_name_ar' => $request->p_name_ar,
